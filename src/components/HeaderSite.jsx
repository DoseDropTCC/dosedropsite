...
export function HeaderSite () {
    ...
    return (
        <header className='font-erica flex justify-between items-center bg-vinho-um h-20 md:h-35 w-full md:w-full'>
                <nav className='flex items-center'>
                    <a href="/"> <img src={doselogom} alt="Dose Drop Logo Mobile" className='flex md:hidden h-25 w-35' /> </a>
                    <a href="/"> <img src={doselogo} alt=" Dose Drop logo" className='hidden md:flex font-erica md:rounded-3xl h-25 rounded-xl md:h-22 md:ml-10 m-5 hover:size-20 duration-150 ease-in cursor-pointer'/></a>
                    <a href="/"><h1 className='hover:text-amarelo-claro  transition-colors duration-350 text-2xl md:text-7xl md:ml-100 md:cursor-pointer text-amarelo' onClick={() => setOpen(!open)}>DoseDrop</h1>
                    </a>
                </nav>
            ...
            <nav className='flex p-5 mr-10'>
                ...
                <button><a href="/"><img src={menuA} alt="menu mobile" className='h-15 w-20 block md:hidden'/></a></button>
            </nav>
        </header>
    )
}